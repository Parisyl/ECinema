<template>
  <div class="header-container">
    <el-menu
      ...
      <el-menu-item v-for="item in items" :key="item.id" :index="item.id" @click="onClick(item.path)">
        {{ item.name }}
      </el-menu-item>

      <el-sub-menu v-for="sub in subs" :key="sub.id" :index="sub.id">
        <template #title>{{ sub.name }}</template>
        <el-menu-item v-for="list in sub.lists" :key="list.id" :index="list.id" @click="onClick(list.path)">
          {{ list.name }}
        </el-menu-item>
      </el-sub-menu>
